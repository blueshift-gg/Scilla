use {
    crate::{
        ...
        prompt::{
            prompt_build_mode, prompt_confirm_install, prompt_input_data, prompt_select_data,
        },
        ui::show_spinner,
    },
    anyhow::anyhow,
...
const SBPF_CONFIG_TOML: &str = r#"[unstable]
...
impl BuildCommand {
    fn spinner_msg(&self) -> &'static str {
        match self {
            BuildCommand::Build => "Building program for sbpf target...",
            BuildCommand::GoBack => "Going back...",
        }
    }
}
...
impl BuildPlan {
    fn for_mode(mode: BuildMode) -> Self {
        match mode {
...
impl Command for BuildCommand {
    async fn process_command(&self, ctx: &mut ScillaContext) -> anyhow::Result<CommandFlow> {
        ctx.get_nav_context_mut()
            .checked_push(NavigationSection::Build);
        match self {
            BuildCommand::Build => {
                println!(
                    "{}",
                ...
                let plan = BuildPlan::for_mode(build_mode);
                let build_context = prepare_build(program_dir, plan)?;
                show_spinner(self.spinner_msg(), run_build(build_context, plan)).await;
            }
            BuildCommand::GoBack => {
...
fn ensure_cargo_tool(check_cmd: &str, crate_name: &str, description: &str) -> anyhow::Result<()> {
    if has_command_version(check_cmd)? {
        return Ok(());
    }

    let prompt_msg =
        format!("Install {crate_name}? Required {description}. (cargo install {crate_name})");
    if !prompt_confirm_install(&prompt_msg) {
        return Err(anyhow!(
            "{crate_name} is required. Install it with: cargo install {crate_name}"
        ));
    }
    ...
    let install_status = ProcessCommand::new("cargo")
        ...
        .status()
        .map_err(|err| anyhow!("Failed to install {crate_name}: {err}"))?;

    if !install_status.success() {
        return Err(anyhow!(
            "{crate_name} install failed. Run: cargo install {crate_name}"
        ));
    }
...
fn ensure_sbpf_linker() -> anyhow::Result<()> {
    ensure_cargo_tool(
        "sbpf-linker",
        "sbpf-linker",
        "to build upstream sbpf programs",
    )
}

fn ensure_llvm(platform: Platform) -> anyhow::Result<()> {
    if has_command_version("llvm-config")? {
        return Ok(());
    }

    match platform {
        Platform::Macos => install_llvm_macos()?,
        Platform::Linux => install_llvm_linux()?,
    }
...
fn ensure_cargo_sbf() -> anyhow::Result<()> {
    ...
    if !command_exists("cargo-build-sbf") {
        return Err(anyhow!(
            "cargo build-sbf is still unavailable. Ensure cargo-build-sbf is on PATH."
        ));
    }
...
fn detect_platform() -> anyhow::Result<Platform> {
    match env::consts::OS {
        ...
        "windows" if is_wsl() => Ok(Platform::Linux),
        os => Err(anyhow!(
            "Unsupported OS for automatic setup: {os}. Use macOS, Linux, or WSL and install \
...
fn install_llvm_macos() -> anyhow::Result<()> {
    if !has_command_version("brew")? {
        return Err(anyhow!(
            "Homebrew is required to install LLVM. Install it from https://brew.sh or handle it \
             manually."
        ));
    }

    if !prompt_confirm_install(
        "Install LLVM? Required to build upstream sbpf programs. (brew install llvm)",
    ) {
        return Err(anyhow!(
            "LLVM install is required, install it with: brew install llvm"
        ));
    }

    let install_status = ProcessCommand::new("brew")
        .args(["install", "llvm"])
        .status()
        .map_err(|err| anyhow!("Failed to install LLVM: {err}"))?;

    if !install_status.success() {
        return Err(anyhow!(
            "LLVM install failed. Install it with: brew install llvm"
        ));
    }
...
struct LinuxPackageManager {
    ...
    prompt_cmd: &'static str,
}

const LINUX_PACKAGE_MANAGERS: &[LinuxPackageManager] = &[
    LinuxPackageManager {
        check_cmd: "apt-get",
        pre_install: Some(&["sudo", "apt-get", "update"]),
        install_args: &["sudo", "apt-get", "install", "-y", "llvm"],
        prompt_cmd: "sudo apt-get install -y llvm",
    },
    LinuxPackageManager {
        check_cmd: "dnf",
        pre_install: None,
        install_args: &["sudo", "dnf", "install", "-y", "llvm"],
        prompt_cmd: "sudo dnf install -y llvm",
    },
    LinuxPackageManager {
        check_cmd: "pacman",
        pre_install: None,
        install_args: &["sudo", "pacman", "-Sy", "--noconfirm", "llvm"],
        prompt_cmd: "sudo pacman -Sy --noconfirm llvm",
    },
];

fn install_llvm_linux() -> anyhow::Result<()> {
    let run_install = |args: &[&str], description: &str| -> anyhow::Result<()> {
        let status = ProcessCommand::new(args[0])
            .args(&args[1..])
            .status()
            .map_err(|err| anyhow!("Failed to run {description}: {err}"))?;
        if !status.success() {
            return Err(anyhow!("{description} failed."));
        }
        Ok(())
    };

    for pm in LINUX_PACKAGE_MANAGERS {
        if !command_exists(pm.check_cmd) {
            continue;
        }

        let prompt_msg = format!(
            "Install LLVM? Required to build upstream sbpf programs. ({})",
            pm.prompt_cmd
        );
        if !prompt_confirm_install(&prompt_msg) {
            return Err(anyhow!("LLVM install skipped."));
        }

        if let Some(pre_install) = pm.pre_install {
            run_install(pre_install, &format!("{} update", pm.check_cmd))?;
        }

        return run_install(pm.install_args, &format!("{} install llvm", pm.check_cmd));
    }

    Err(anyhow!(
        "No supported Linux package manager found. Install LLVM manually."
    ))
}
...
fn ensure_cargo_config(config_root: &Path) -> anyhow::Result<()> {
    ...
    fs::write(&config_path, SBPF_CONFIG_TOML)?;
    Ok(())
}
